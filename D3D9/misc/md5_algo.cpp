...
/*
 * Update from brliron : these functions will be called from the DllMain function on Windows.
 * At this point, there is no guarantee the libc will be available.
 * So we'll use our own memset and memcpy.
 */
static void *memset(void *s, int c, unsigned long n)
{
	unsigned char *ptr;

	for (ptr = (unsigned char*)s; n > 0; ptr++, n--)
		*ptr = c;
	return s;
}
...
static void *memcpy(void *dest, const void *src, unsigned long n)
{
	...
	for (ptr = (unsigned char*)dest; n > 0; ptr++, src_++, n--)
		*ptr = *src_;
	return dest;
}
